...
public class JipiaoOrderController {
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request) {
        ...
        if (jipiaoOrder != null) {
            ...
            if (jipiao != null) {
                BeanUtils.copyProperties(jipiao, view, "id", "createTime", "insertTime", "updateTime", "yonghuId");//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if (yonghu != null) {
                BeanUtils.copyProperties(yonghu, view, "id", "createTime", "insertTime", "updateTime", "yonghuId");//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//                            jipiaoOrderEntity.setCreateTime(date);//时间
                        ...
                        if (jipiaoOrderEntities_jipiaoOrderUuidNumber.size() > 0) {
                            ...
                            return R.error(511, "数据库的该表中的 [订单号] 字段已经存在 存在数据为:" + repeatFields);
                        }
    ...
    public R detail(@PathVariable("id") Long id, HttpServletRequest request) {
        ...
        if (jipiaoOrder != null) {
            ...
            JipiaoEntity jipiao = jipiaoService.selectById(jipiaoOrder.getJipiaoId());
            if (jipiao != null) {
                BeanUtils.copyProperties(jipiao, view, "id", "createDate");//把级联的数据添加到view中,并排除id和创建时间字段
                view.setJipiaoId(jipiao.getId());
            }
            //级联表
            YonghuEntity yonghu = yonghuService.selectById(jipiaoOrder.getYonghuId());
            if (yonghu != null) {
                BeanUtils.copyProperties(yonghu, view, "id", "createDate");//把级联的数据添加到view中,并排除id和创建时间字段
                view.setYonghuId(yonghu.getId());
            }
            //修改对应字典表字段
            dictionaryService.dictionaryConvert(view, request);
    ...
    public R add(@RequestBody JipiaoOrderEntity jipiaoOrder, HttpServletRequest request) {
        ...
        if (buyZuoweiNumberList != null && buyZuoweiNumberList.size() > 0) {
            return R.error(511, buyZuoweiNumberList + " 的座位已经被使用");
        }
